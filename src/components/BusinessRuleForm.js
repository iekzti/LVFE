...
function BusinessRuleForm({ isShow, onClose, BPs }) {
  useEffect(() => {
    if (BPs?.length > 0) {
      setForm((prev) => ({
        ...prev,
        name: `Luật nghiệp vụ hỗ trợ cho ${BPs[0].name}`,
        businessProcessId: BPs[0].id
      }));
    }
  }, [BPs]);
  ...
  const handleChangeName = (e) => {
    ...
    setForm((prev) => ({
      ...prev,
      name: `Luật nghiệp vụ hỗ trợ cho ${bp.name}`,
  ...
  const handleSave = async () => {
    ...
    if (resultUploadFile.status === 201) {
      ...
      const result = await axios
        .post(
          endpoint,
          {
            businessProcessId: form.businessProcessId,
            name: form.name,
            topic: form.topic,
  ...
  return (
    ...
    >
      ...
      >
        Vui lòng chọn tên cho luật nghiệp vụ:
      </div>
      <div style={styles.inputContainer}>
        ...
        >
          {BPs.map((bp) => (
            <option value={JSON.stringify(bp)} >{`Luật nghiệp vụ hỗ trợ cho ${bp.name}`}</option>
          ))}
        </select>
      </div>
      {/* -------------------------------------------------------------------------- */
